...
export class FileService {
    ...
    constructor(private configService: ConfigService) {
        ...
        this.s3Client = new S3Client({
            region: "us-east-1",
            endpoint:
                this.configService?.get("DO_SPACE_ENDPOINT") ||
                process.env.DO_SPACE_ENDPOINT,
            credentials: {
                accessKeyId:
                    this.configService?.get("DO_SPACE_ACCESS_KEY") ||
                    process.env.DO_SPACE_ACCESS_KEY ||
                    "",
                secretAccessKey:
                    this.configService?.get("DO_SPACE_SECRET_KEY") ||
    ...
    /**
     * Uploads a single file (or local path/URL) to Cloudinary and returns its secure URL.
     */
